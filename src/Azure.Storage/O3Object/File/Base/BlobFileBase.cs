...
internal abstract class BlobFileBase<T, S> : FileBase, ConnectionString.IHasNameAndKey
    where T : IBlobBaseClient
    where S : BlobFileOption, new()
{
    protected S? _analyzedResult;
    ...
    protected override Task OnInitializingAsync(JsonElement option, bool needVersionCheck, object? fromParent, CancellationToken token)
    {
        ...
        _tempDir = temp.GetValue(1) as DirectoryInfo;
    ...
    protected virtual S AnalyzeOption(JsonElement option)
    {
        var optionManager = new OptionManager<S>(option);
    ...
    protected async override Task OnOpeningAsync(JsonElement rootElement, IDictionary<string, object?> argument, CancellationToken token)
    {
        try
        {
            ...
            _tempFile = new FileInfo(Path.Combine(_tempDir!.FullName, tempFileName));
    ...
    protected abstract T GetClient(BlobContainerClientEx client, string blobPath);
    ...
    private Task<IDictionary<string, object?>> GetPropertiesAsync(IDictionary<string, object?> _1, CancellationToken _2)
    {
        if (_stream == null)
        {
            throw new ProviderCoreException(ResultCode.FileNotOpened);
